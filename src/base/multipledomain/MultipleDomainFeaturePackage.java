...
?@     w      t Calc2.all&&Calc1.all&&Calc.allsq ~ w   sq ~ t Calc2t {    st  . finishes ()  ; }t exitsq ~ t Calc1t {    st  . finishes ()  ; }t exitsq ~ q ~ t {    st  . finishes ()  ; }t exitxxsq ~ w   q ~ xxt minus.Minussq ~ ?@     w      t allsq ~ 
?@     w      t 	Calc2.allsq ~ w   sq ~ q ~ t X{    total  . add ( - e  . getValue ())  ;   st  . setResult (total  . intValue ())  ; }t minusxxsq ~ w   q ~ +xxt entersq ~ ?@     w      t allsq ~ 
?@     w      t +Calc2.MinusDomain.i.equalsTo(MinusDomain.j)sq ~ w   sq ~ q ~ t ,{    System  . out  . println ("test2")  ; }t multipleDomainxt 1Calc1.MultiplyDomain.i.equalsTo(MultiplyDomain.j)sq ~ w   sq ~ q ~  t +{    System  . out  . println ("test")  ; }t multipleDomainxt ^Calc2.MinusDomain.i.equalsTo(MinusDomain.j)&&Calc1.MultiplyDomain.i.equalsTo(MultiplyDomain.j)sq ~ w   q ~ 7q ~ <xxsq ~ w   q ~ ?q ~ :q ~ 5xxt multiply.Multiplysq ~ ?@     w      q ~ 3sq ~ 
?@     w      t 	Calc1.allsq ~ w   sq ~ q ~  t «{    int current = total  . intValue ()  ; for (  int i = 1 ; i  < e  . getValue ();  i ++ ) {    total  . add (current)  ; }   st  . setResult (total  . intValue ())  ; }t multiplyingxxsq ~ w   q ~ Exxxt MultipleDomainfeature
*/
